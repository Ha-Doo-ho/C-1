...
#include<stdlib.h>
#include<string.h>
/*단순 연결 리스트 plist3을 두 개의 단순 연결 리스트 plist1과 plist2로 분리하느 함수 split. plist3의 홀수 번째 노드들은 모두 plist1로 이동되고 
plist3의 짝수 번째 노드들은 모두 plist2로 이동된다. 이 함수가 plist3을 변경하여서는 안 된다. split함수 시간복잡도 : O(n)*/
